...
export default function MovieDetailClient({ movie }: { movie: Movie }) {
  ...
  useEffect(() => {
    const incrementViews = async () => {
      try {
        if (movie && movie.slug) {
          // Check if we've already viewed this movie in this session
          const viewedMovies = JSON.parse(
            sessionStorage.getItem('viewedMovies') || '[]'
          );
          if (!viewedMovies.includes(movie.slug)) {
            // Increment view count
            await incrementMovieViewCount(movie.slug);
            // Add to viewed movies
            viewedMovies.push(movie.slug);
